...
async function getDemoUserId() {
    ...
    try {
        const { rows } = await client.query("SELECT id FROM users WHERE email = $1", [DEMO_EMAIL]);
        if (rows.length === 0) {
            // fallback: create row if seed not run
            const insert = await client.query("INSERT INTO users (email, password_hash) VALUES ($1, $2) RETURNING id", [DEMO_EMAIL, ""]);
...
app.post("/budget/sync", async (req, res) => {
    ...
    const { income = 0, bills = 0, food = 0, transport = 0, subscriptions = 0, misc = 0, } = budget;
    const client = await pool.connect();
    try {
